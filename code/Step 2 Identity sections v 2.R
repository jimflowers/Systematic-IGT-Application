...
code_sec_str4<-"^\\s+\\w+\\-\\w+\\-\\w+\\.|^\\s=|\"\\w+\\-\\w+\\-\\w+\\.w?\\w?"
...
while (i<=tot.sections){
  if (grepl("CHAPTER",test_frame$text.var[beg.quote[i]])){
    ocga_tca<-trimws(test_frame$text.var[beg.quote[i]+1])
  }else{
  ...
  while (j<=j_array) {
    ...
    if (j==j_array) {
      s_frame$OCGA[OCGA_array[j]:sframe_end]<-str_extract(s_frame$text.var[OCGA_array[j]],code_sec_str4)
    } else {
      s_frame$OCGA[OCGA_array[j]:OCGA_array[j+1]]<-str_extract(s_frame$text.var[OCGA_array[j]],code_sec_str4)
    }
...
# ocga_new<-grep(ocga_all,t_frame$text.var)
#ocga_com<-paste(ocga_all,code_sec_str2,sep="|^")
ocga_new<-str_extract(t_frame$text.var,ocga_all)
t_frame$ocga_sub<-trimws(ocga_new)

#if (t_frame$OCGA_SUB == "NA" { t_frame$OCGA_SUB<-[this record look back 1]})
#if (ocga_new !="NA") {
#  t_frame$OCGA_SUB<-ocga_new
# } else {
 # t_frame$OCGA_SUB<-t_frame$OCGA_SUB[-1]
 # }
...
## i erased this attemtp - 5 jan 2020
...
while (s<=numsec)  {
  ...
  # sec<-substr(trimws(t_frame$text.var[start]),1,3) #the section id  and this doesn not find (a)(1)
