...
section_array<-grep("SECTION (\\d)",test_frame$text.var) 
section_matrix<-str_match(test_frame$text.var,"SECTION (\\d)")# finds outline indicator for bill section 1,2,...
...
tot.sections<-length(section_array)
...
while (i<=tot.sections){
  ...
  if (length(section_array==1)){
    s_frame<-cbind(test_frame[(section_array[i]):nrow(test_frame),],"Section"=str_match(test_frame$text.var,"SECTION (\\d)")[i,2],"OCGA"=ocga_tca,"OCGA_SUB"=" ",stringsAsFactors=F)
    
  } else {
  s_frame<-cbind(test_frame[(section_array[i]):(section_array[i+1]-1),],"Section"=str_match(test_frame$text.var,"SECTION (\\d)")[i,2],"OCGA"=ocga_tca,"OCGA_SUB"=" ",stringsAsFactors=F) #modified 12/2/19 to capture Section
  } 
   # Find OCGA changes within Bill Sections
  period_present<-grepl("\\.$|:$|\\'$",s_frame$text.var)
  ...
  s_frame$text.var<-gsub("\"","",s_frame$text.var)  # remove quotes from beginning of input lines
  ocga_new<-str_extract(s_frame$text.var,ocga_all)
  sub_sec_array<-grep(ocga_all,s_frame$text.var)
  sub_sec_present<-grepl("\\;$|\\:$|\\.$|or$|and$",trimws(s_frame$text.var))
  
  sub_sec_array<-keep(sub_sec_array,sub_sec_present[sub_sec_array-1])
  sub_sec_array<-sort(c(OCGA_array, sub_sec_array))
  
  

  
  k<-1
  array_end<-length(sub_sec_array)
  ocga_var<-s_frame$OCGA[1]
  while (k<=array_end){
    if (k==array_end) {
      s_frame$OCGA_SUB[sub_sec_array[k]:nrow(s_frame)]<-ocga_new[sub_sec_array[k]]
    } else {
      s_frame$OCGA_SUB[sub_sec_array[k]:(sub_sec_array[k+1]-1)]<-ocga_new[sub_sec_array[k]]
    }
    k<-k+1
...
tx<-grep(ocga_all,t_frame$text.var)  # builds a vector of OCGA section and subsection changes
new_section<-grep("SECTION",t_frame$text.var) # need new vector of bill sections as the file is truncated
tx<-sort(c(tx,new_section)) #get my section changes in order
...
while (s<=numsec)  {
  ...
  sec_sub<-t_frame$ocga_sub[start]  #make sure the srch_str is the right pattern
  Section_var<-t_frame$Section[start]
  ...
  sec_sub<-t_frame$OCGA_SUB[start]
 ...
 # ti_ch_sec<-str_extract(t_frame$text.var[i],code_sec_str4)
   # build the record and bind it to the dataframe
    b<-data.frame("Title_Chap"=ti_ch_sec ,"Sub_section"=sec_sub,"Bill_Section"=Section_var,"Observation"=test,"Doc Id"=1,"Obs.No"=s,"Start"=start,"End"=nxt,stringsAsFactors = F)  # a quick dataframe to be appended to the main data frame
